...
void print_hex(u8 *buffer, u16 buffer_len){
    int i;
	if(buffer[0] == NULL) printf("pos vaya una mierda\n");
	//printf("ENTRAMOS A PRINT HEX\n");
...
u16 build_message2( u8 *Kbs, u16 Kbs_len,
    ...
    //poner lo del hash
	id_A = (u8 *) malloc(SHA_DIGEST_LENGTH*sizeof(u8));
	u16 id_A_len = 20; //20 bytes hash size
	printf("antes de hacer el hash\n");
	fflush(stdout);

	struct timeval t1,t2;
	gettimeofday(&t1, NULL);
	SHA1(message1, message1_len,id_A);
	gettimeofday(&t2, NULL);
	double secs = timeval_diff(&t2, &t1);
	printf("%.16g milliseconds\n", secs * 1000.0);
	printf("VALOR DEL HASH  ");
	print_hex(id_A, SHA_DIGEST_LENGTH);
	printf("despues de hacer el hash\n");
	fflush(stdout);
    ...
    *message2_ptr = message2;
	printf("LONGITUD DENTRO DE BUILD2 %d\n",message2_len);
    return message2_len;
...
u16 build_message2_extended( u8 *Kbl, u16 Kbl_len,
    ...
    u16 message1_pos = 0;
    u8 *id_A = (u8 *) malloc(SHA_DIGEST_LENGTH*sizeof(u8));
    u16 id_A_len = SHA_DIGEST_LENGTH;//get_payload(message1, &message1_pos, &id_A);
    SHA1(message1, message1_len,id_A);

    u8 *id_B ="EQUIPO B";
    u16 id_B_len = 9; //get_payload(message1, &message1_pos, &id_B);
