	...
	if (eap_sm_Policy_doPickUp(sm, sm->respMethod)) {
		...
		if (sm->m && sm->m->initPickUp) {
			...
			if (sm->eap_method_priv == NULL) {
				printf( "EAP: Failed to "
					   "initialize EAP method %d",
					   sm->currentMethod);
				printf( "EAP: Failed to initialize EAP method %d\n",
					   sm->currentMethod);
				sm->m = NULL;
				sm->currentMethod = EAP_TYPE_NONE;
			}
...
SM_STATE(EAP, METHOD_REQUEST)
{
	...
	if (sm->m == NULL) {
		printf( "EAP: method not initialized\n");
		return;
	}

	sm->currentId = eap_sm_nextId(sm, sm->currentId);
	//printf( "EAP: building EAP-Request: Identifier %d\n",
	//	   sm->currentId);
...
SM_STATE(EAP, PROPOSE_METHOD)
{
	...
	if (sm->m) {
		//printf("He obtenido bien el método.\n");		
		sm->eap_method_priv = sm->m->init(sm);
		if (sm->eap_method_priv == NULL) {
			printf("EAP: Failed to initialize EAP "
				   "method %d\n", sm->currentMethod);
			sm->m = NULL;
			sm->currentMethod = EAP_TYPE_NONE;
		}
...
SM_STEP(EAP)
{
	...
	case EAP_SEND_REQUEST:
		//printf ("EAP_SEND_REQUEST\n");
		SM_ENTER(EAP, IDLE);
		break;
	case EAP_INTEGRITY_CHECK:
		//printf ("EAP_INTEGRITY_CHECK\n");
		if (sm->ignore)
			SM_ENTER(EAP, DISCARD);
		else
			SM_ENTER(EAP, METHOD_RESPONSE);
	...
	case EAP_METHOD_REQUEST:
		//printf ("EAP_METHOD_REQUEST\n");
		SM_ENTER(EAP, SEND_REQUEST);
		break;
	case EAP_METHOD_RESPONSE:
		//printf ("EAP_METHOD_RESPONSE\n");
		/*
	...
	case EAP_INITIALIZE_PASSTHROUGH:
		//printf ("EAP_INITIALIZE_PASSTHROUGH\n");
		if (sm->currentId == -1)
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
	...
	if (resp == NULL || wpabuf_len(resp) < sizeof(*hdr)) {
		//printf( "EAP: parseEapResp: invalid resp=%p "
		//	   "len=%lu", resp,
	...
	if (plen > wpabuf_len(resp)) {
		printf( "EAP: Ignored truncated EAP-Packet "
			   "(len=%lu plen=%lu)",
	...
	//printf( "EAP: parseEapResp: rxResp=%d respId=%d "
...
static int eap_sm_getId(const struct wpabuf *data)
{
	...
	//printf( "EAP: getId: id=%d", hdr->identifier);
...
static struct wpabuf * eap_sm_buildSuccess(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	//printf( "EAP: Building EAP-Success (id=%d)", id);
...
static struct wpabuf * eap_sm_buildFailure(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	//printf( "EAP: Building EAP-Failure (id=%d)", id);
...
void eap_sm_process_nak(struct eap_sm *sm, const u8 *nak_list, size_t len)
{
    ...
    //printf( "EAP: processing NAK (current EAP method "
	 //	   "index %d)\n", sm->user_eap_method_index);
    
    int ii;
/*
    for (ii=0; ii < EAP_MAX_METHODS * sizeof(sm->user->methods[0]); ii++){
        printf("%X:",*(((u8 *) sm->user->methods)+ii));
    }
    printf("\n");
    for (ii=0; ii < len; ii++){
        printf("%X:",*(((u8 *)  nak_list)+ii));
    }
    printf("\n");    
*/
				 ...
				 size_t len)
{
	...
	if (sm->user->phase2) {
		printf( "EAP: EAP-Nak received after Phase2 user"
...
//	 printf("ENTRNADO AQUI\n");	
	 if (sm->identity == NULL && sm->currentMethod == EAP_TYPE_FRM) {
                *vendor = EAP_VENDOR_IETF;
                next  = EAP_TYPE_TLS;
//                next  = EAP_TYPE_IDENTITY;
                sm->update_user = TRUE;
//printf(" 1\n");
        } else	if (sm->identity == NULL || sm->currentId == -1) {		
		*vendor = EAP_VENDOR_IETF;
		next = EAP_TYPE_FRM;
                sm->update_user = TRUE;
		//next = EAP_TYPE_IDENTITY;
...
//	printf("EAP: getNextMethod: vendor %d type %d\n",
//		   *vendor, next);
	return next;
}
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
	...
	if (sm->m && sm->m->isDone(sm, sm->eap_method_priv) &&
	    !sm->m->isSuccess(sm, sm->eap_method_priv)) {
		printf( "EAP: getDecision: method failed -> "
			   "FAILURE");
		sm->update_user = TRUE;
	...
	if ((sm->user == NULL || sm->update_user) && sm->identity) {
		if (eap_user_get(sm, sm->identity, sm->identity_len, 0) != 0) {
			printf( "EAP: getDecision: user not "
	...
	if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
	    (sm->user->methods[sm->user_eap_method_index].vendor !=
	     ...
	     EAP_TYPE_NONE)) {
		printf( "EAP: getDecision: another method "
			   "available -> CONTINUE");
		return DECISION_CONTINUE;
	}

	if (sm->identity == NULL || sm->currentId == -1) {
		//printf( "EAP: getDecision: no identity known "
		//	   "yet -> CONTINUE");
		return DECISION_CONTINUE;
	}

	printf( "EAP: getDecision: no more methods available -> "
		   "FAILURE");
	return DECISION_FAILURE;
}
...
//	printf( "EAP: Server state machine created");
...
void eap_server_sm_deinit(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
//	printf( "EAP: Server state machine removed");
	if (sm->m && sm->eap_method_priv)
		sm->m->reset(sm, sm->eap_method_priv);
	wpabuf_free(sm->eap_if.eapReqData);
	os_free(sm->eap_if.eapKeyData);
...
void eap_sm_pending_cb(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
//	printf( "EAP: Callback for pending request received");
	if (sm->method_pending == METHOD_PENDING_WAIT)
