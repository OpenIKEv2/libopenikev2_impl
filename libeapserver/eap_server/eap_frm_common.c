...
#ifdef EAP_FRM_SHA256
#include "./crypto/sha256.h"
#endif /* EAP_FRM_SHA256 */
#include "eap_frm_common.h"


/**
 * eap_frm_supported_ciphersuite - Check whether ciphersuite is supported
...
{
	if (vendor == EAP_FRM_VENDOR_IETF &&
	    specifier == EAP_FRM_CIPHER_AES)
		return 1;
#ifdef EAP_FRM_SHA256
	if (vendor == EAP_FRM_VENDOR_IETF &&
	    specifier == EAP_FRM_CIPHER_SHA256)
		return 1;
#endif /* EAP_FRM_SHA256 */
	return 0;
}
...
static int eap_frm_gkdf_cmac(const u8 *psk /* Y */,
...
#ifdef EAP_FRM_SHA256
static int eap_frm_gkdf_sha256(const u8 *psk /* Y */,
				const u8 *data /* Z */, size_t data_len,
				u8 *buf, size_t len /* X */)
{
	u8 *opos;
...
static int eap_frm_derive_keys_helper(u32 csuite_specifier,
				       ...
				       u8 *pk, size_t pk_len)
{
	...
	case EAP_FRM_CIPHER_AES:
		gkdf = eap_frm_gkdf_cmac;
		mk_len = 16;
		break;
#ifdef EAP_FRM_SHA256
	case EAP_FRM_CIPHER_SHA256:
		gkdf = eap_frm_gkdf_sha256;
	...
	pos += psk_len;
	WPA_PUT_BE32(pos, EAP_FRM_VENDOR_IETF); /* CSuite/Vendor = IETF */
	pos += 4;
	WPA_PUT_BE16(pos, csuite_specifier); /* CSuite/Specifier */
	pos += 2;
	os_memcpy(pos, seed, seed_len); /* inputString */
	wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: Data to MK derivation",
			data, data_len);
	...
	os_free(data);
	wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: MK", mk, mk_len);

	if (gkdf(mk, seed, seed_len, kdf_out, kdf_out_len) < 0)
		return -1;

	pos = kdf_out;
	wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: MSK", pos, EAP_MSK_LEN);
	os_memcpy(msk, pos, EAP_MSK_LEN);
	pos += EAP_MSK_LEN;

	wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: EMSK", pos, EAP_EMSK_LEN);
	os_memcpy(emsk, pos, EAP_EMSK_LEN);
	pos += EAP_EMSK_LEN;

	wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: SK", pos, sk_len);
	os_memcpy(sk, pos, sk_len);
	pos += sk_len;

	if (pk) {
		wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: PK", pos, pk_len);
...
static int eap_frm_derive_keys_aes(const u8 *psk, size_t psk_len,
				    const u8 *seed, size_t seed_len,
				    u8 *msk, u8 *emsk, u8 *sk, size_t *sk_len,
				    u8 *pk, size_t *pk_len)
{
#define EAP_FRM_SK_LEN_AES 16
#define EAP_FRM_PK_LEN_AES 16
	u8 kdf_out[EAP_MSK_LEN + EAP_EMSK_LEN + EAP_FRM_SK_LEN_AES +
		   EAP_FRM_PK_LEN_AES];
	...
	*sk_len = EAP_FRM_SK_LEN_AES;
	*pk_len = EAP_FRM_PK_LEN_AES;

	return eap_frm_derive_keys_helper(EAP_FRM_CIPHER_AES,
...
#ifdef EAP_FRM_SHA256
static int eap_frm_derive_keys_sha256(const u8 *psk, size_t psk_len,
				       const u8 *seed, size_t seed_len,
				       u8 *msk, u8 *emsk,
				       u8 *sk, size_t *sk_len)
{
#define EAP_FRM_SK_LEN_SHA256 SHA256_MAC_LEN
#define EAP_FRM_PK_LEN_SHA256 SHA256_MAC_LEN
	u8 kdf_out[EAP_MSK_LEN + EAP_EMSK_LEN + EAP_FRM_SK_LEN_SHA256 +
		   EAP_FRM_PK_LEN_SHA256];
	...
	*sk_len = EAP_FRM_SK_LEN_SHA256;

	return eap_frm_derive_keys_helper(EAP_FRM_CIPHER_SHA256,
					   kdf_out, sizeof(kdf_out),
					   psk, psk_len, seed, seed_len,
					   msk, emsk, sk, *sk_len,
					   NULL, 0);
...
/**
 * eap_frm_derive_keys - Derive EAP-FRM keys
 * @psk: Pre-shared key
 ...
 * @sk: Buffer for SK (at least EAP_FRM_MAX_SK_LEN bytes)
 * @sk_len: Buffer for returning length of SK
 * @pk: Buffer for PK (at least EAP_FRM_MAX_PK_LEN bytes)
			 ...
			 u8 *pk, size_t *pk_len)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-FRM: Deriving keys (%d:%d)",
		   vendor, specifier);

	if (vendor != EAP_FRM_VENDOR_IETF)
		return -1;

	wpa_hexdump_key(MSG_DEBUG, "EAP-FRM: PSK", psk, psk_len);

	/* Seed = RAND_Peer || ID_Peer || RAND_Server || ID_Server */
	seed_len = 2 * EAP_FRM_RAND_LEN + id_server_len + id_peer_len;
	seed = os_malloc(seed_len);
	if (seed == NULL) {
		wpa_printf(MSG_DEBUG, "EAP-FRM: Failed to allocate memory "
			   "for key derivation");
		return -1;
	}

	pos = seed;
	os_memcpy(pos, rand_peer, EAP_FRM_RAND_LEN);
	pos += EAP_FRM_RAND_LEN;
	os_memcpy(pos, id_peer, id_peer_len);
	pos += id_peer_len;
	os_memcpy(pos, rand_server, EAP_FRM_RAND_LEN);
	pos += EAP_FRM_RAND_LEN;
	os_memcpy(pos, id_server, id_server_len);
	pos += id_server_len;
	wpa_hexdump(MSG_DEBUG, "EAP-FRM: Seed", seed, seed_len);

	switch (specifier) {
	case EAP_FRM_CIPHER_AES:
		ret = eap_frm_derive_keys_aes(psk, psk_len, seed, seed_len,
					       msk, emsk, sk, sk_len,
					       pk, pk_len);
		break;
#ifdef EAP_FRM_SHA256
	case EAP_FRM_CIPHER_SHA256:
		ret = eap_frm_derive_keys_sha256(psk, psk_len, seed, seed_len,
						  msk, emsk, sk, sk_len);
		break;
#endif /* EAP_FRM_SHA256 */
	default:
		wpa_printf(MSG_DEBUG, "EAP-FRM: Unknown cipher %d:%d used in "
...
/**
 * eap_frm_mic_len - Get the length of the MIC
 * @vendor: CSuite/Vendor
 * @specifier: CSuite/Specifier
...
static int eap_frm_compute_mic_aes(const u8 *sk, size_t sk_len,
				    const u8 *data, size_t len, u8 *mic)
{
	if (sk_len != 16) {
		wpa_printf(MSG_DEBUG, "EAP-FRM: Invalid SK length %d for "
			   "AES-CMAC MIC", sk_len);
		return -1;
	}
...
/**
 * eap_frm_compute_mic - Compute EAP-FRM MIC for an EAP packet
 * @sk: Session key SK from eap_frm_derive_keys()
 * @sk_len: SK length in bytes from eap_frm_derive_keys()
 * @vendor: CSuite/Vendor
 * @specifier: CSuite/Specifier
 * @data: Input data to MIC
 * @len: Input data length in bytes
 * @mic: Buffer for the computed MIC, eap_frm_mic_len(cipher) bytes
 * Returns: 0 on success, -1 on failure
 */
			 ...
			 int specifier, const u8 *data, size_t len, u8 *mic)
{
	...
	switch (specifier) {
	case EAP_FRM_CIPHER_AES:
		ret = eap_frm_compute_mic_aes(sk, sk_len, data, len, mic);
		break;
#ifdef EAP_FRM_SHA256
	case EAP_FRM_CIPHER_SHA256:
