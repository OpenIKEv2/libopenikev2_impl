...
struct ip6_mh_opt_bid {
	...
	uint8_t		ip6mobid_status;
	uint8_t		ip6mobid_flags;
	struct in6_addr	ip6mobid_addr;
} __attribute__((packed));

struct ip6_mh_opt_bid_noaddr {
	uint8_t		ip6mobid_type;
	uint8_t		ip6mobid_len;
	uint16_t	ip6mobid_bid;
	uint8_t		ip6mobid_status;
...
#define IP6_MHOPT_BID		0x23	/* Binding Unique Identifier */
...
#define IP6_MH_BAS_PRFX_DISCOV		1	/* Accepted, but prefix discovery
						   required */
#define IP6_MH_BAS_MCOA_NOTCOMP		4	/* MCoA not complete */
#define IP6_MH_BAS_MCOA_RETURNHOME	5	/* MCoA Return home wo/ndp */
...
#define IP6_MH_BAS_MCOA_MALFORMED	164	/* MCoA malformed */
#define IP6_MH_BAS_CONFLICT_BINDING	165	/* MCoA Conflict between bindings:
						   binding exists */
#define IP6_MH_BAS_MCOA_UNKN_COA	167	/* MCoA Unknown CoA */
#define IP6_MH_BAS_MCOA_NOT_PERM	166	/* MCoA prohibited */
#define IP6_MH_BAS_MCOA_BULK_NOT_ALLOW	168	/* Bulk registration prohibited */
#define IP6_MH_BAS_MCOA_HA_FOREG_NOT_A	169	/* MCoA simultaneous home and foreign
