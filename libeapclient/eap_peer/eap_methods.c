...
const struct eap_method * eap_peer_get_eap_method(int vendor, EapType method)
{
	...
	for (m = eap_methods; m; m = m->next) {
		//printf("Por parametro: Vendor=%d,Method=%d, en la estructura: Vendor=%d,Method=%d\n", vendor, method, m->vendor, m->method);
...
int eap_peer_method_load(const char *so)
{
	...
	if (handle == NULL) {
		wpa_printf(MSG_ERROR, "EAP: Failed to open dynamic EAP method "
			   "'%s': %s", so, dlerror());
		printf( "EAP: Failed to open dynamic EAP method "
			   "'%s': %s\n", so, dlerror());
		return -1;
	}
	...
	if (dyn_init == NULL) {
		...
		printf( "EAP: Invalid EAP method '%s' - no "
	...
	if (ret) {
		...
		wpa_printf(MSG_ERROR, "EAP: Failed to add EAP method '%s' - "
			   "ret %d", so, ret);
		printf( "EAP: Failed to add EAP method '%s' - "
			   "ret %d\n", so, ret);
		return ret;
	}
...
{
	printf("Registrando el metodo vendor=%d method=%d name=%s\n", vendor, method, name);
...
#ifdef EAP_EXT
	if (ret == 0) {
		int eap_peer_ext_register(void);
		ret = eap_peer_ext_register();
	}
#endif /* EAP_EXT */

#ifdef EAP_FRM
    if (ret == 0) {
            printf("*****REGISTRANDO EAP_FRM\n");
            int eap_peer_frm_register(void);
            ret = eap_peer_frm_register();
    }
#endif /* EAP_FRM */
...
#ifdef EAP_TPP
        if (ret == 0) {
                int eap_peer_tpp_register(void);
                ret = eap_peer_tpp_register();
        }
#endif /* EAP_TPP */
