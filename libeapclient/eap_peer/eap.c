...
static int eap_allowed_method(struct eap_sm *sm, int vendor, u32 method)
{
	...
	//printf("eap_allowed_method: obtiene un config\n");

	m = config->eap_methods;
	for (i = 0; m[i].vendor != EAP_VENDOR_IETF ||
		     m[i].method != EAP_TYPE_NONE; i++) {

		//printf("eap_allowed_method: %d,%d - %d,%d \n",m[i].vendor,m[i].method, vendor, method);
...
SM_STATE(EAP, GET_METHOD)
{
	...
	if (!eap_sm_allowMethod(sm, sm->reqVendor, method)) {
		wpa_printf(MSG_DEBUG, "EAP: vendor %u method %u not allowed",
			   sm->reqVendor, method);
		printf("EAP: vendor %u method %u not allowed\n",
			   sm->reqVendor, method);
		goto nak;
	}
	...
	if (sm->fast_reauth &&
		...
		printf("EAP: Using previous method data"
			   " for fast re-authentication\n");
		reinit = 1;
	} else {
	...
	if (!sm->m) {
		wpa_printf(MSG_DEBUG, "EAP: Could not find selected method: "
			   "vendor %d method %d",
			   sm->reqVendor, method);
		printf("EAP: Could not find selected method: "
			   "vendor %d method %d\n",
			   sm->reqVendor, method);
		goto nak;
	}
	...
	//printf( "EAP: Initialize selected EAP method: "
	//	   "vendor %u method %u (%s)\n",
	//	   sm->reqVendor, method, sm->m->name);
	if (reinit)
	...
	if (sm->eap_method_priv == NULL) {
		...
		wpa_msg(sm->msg_ctx, MSG_INFO,
			...
			"method %u (%s)",
			sm->reqVendor, method, sm->m->name);
		printf("EAP: Failed to initialize EAP method: vendor %u "
			"method %u (%s)\n",
			sm->reqVendor, method, sm->m->name);
		sm->m = NULL;
		sm->methodState = METHOD_NONE;
		...
		if (sm->reqMethod == EAP_TYPE_TLS && config &&
			...
			wpa_printf(MSG_DEBUG, "EAP: Pending PIN/passphrase "
				   "request - skip Nak");
			printf("EAP: Pending PIN/passphrase "
				   "request - skip Nak\n");
			return;
		}
	...
	wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_METHOD
		...
		sm->reqVendor, method, sm->m->name);
	//printf("EAP vendor %u method %u (%s) selected\n",
	//	sm->reqVendor, method, sm->m->name);
	//printf("9\n");
...
static Boolean eap_frm_is_kerberos_available() {

	    krb5_cc_cursor  cursor;
        krb5_creds  credential;
        krb5_creds my_credential;
        u8 found_realm=0;
...
//delia
 krb5_init_context(&context);
    ...
    /*    retval=krb5_sname_to_principal(context,"kerberos","servicio",KRB5_NT_SRV_HST,&server );
        if(retval){
        ...
        if((retval=krb5_copy_principal(context,server,&my_credential.server))){

            printf("fallo al copiar el principal server en my_credential\n");
            return FALSE;
        }

        if((retval=krb5_copy_principal(context,client,&my_credential.client))){
...
//delia
        //memset(credential,0,sizeof(krb5_creds));

/*        retval=krb5_cc_retrieve_cred(context,ccdef,0,&my_credential,credential);
        if(retval){
            printf("fallo al recuperar el ticket\n");
            return FALSE;
        }
        else{
				  ...
				  EapType method)
{
	if (!eap_allowed_method(sm, vendor, method)) {
		...
		printf("EAP: configuration does not allow: "
			   "vendor %u method %u\n", vendor, method);
		return FALSE;
	}
	//printf("ENTRANDO A ALLOWWW METHODDDD\n");
	...
	wpa_printf(MSG_DEBUG, "EAP: not included in build: "
		   "vendor %u method %u", vendor, method);
	printf("EAP: not included in build: "
		   "vendor %u method %u\n", vendor, method);
	return FALSE;
}
...
static struct wpabuf * eap_sm_buildNak(struct eap_sm *sm, int id)
{
	...
	//printf("EAP: Building EAP-Nak (requested type %u "
	//	   "vendor=%u method=%u not allowed)\n", sm->reqMethod,
	//	   sm->reqVendor, sm->reqVendorMethod);

	methods = eap_peer_get_methods(&count);

	//printf("Count=%d - construir NAK\n", count);
	...
	if (resp == NULL){
                printf("Fallo al reservar memoria para el NAK\n");
		return NULL;
	}
        //printf("Hasta aqui va la cosa bien\n");
...
static void eap_sm_processIdentity(struct eap_sm *sm, const struct wpabuf *req)
{
    ...
    printf("ejecuta eap_sm_processIdentity()\n");
}
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	if (config == NULL) {
		printf( "EAP: buildIdentity: configuration "
			   "was not available\n");
		return NULL;
	}
	...
	if (sm->m && sm->m->get_identity &&
		...
		printf( "EAP: using method re-auth "
				  "identity\n");
		...
		wpa_hexdump_ascii(MSG_DEBUG, "EAP: using real identity",
				  identity, identity_len);
		printf( "EAP: using real id "
				  "identity\n");
	}

	if (identity == NULL) {
		printf( "EAP: buildIdentity: identity "
...
{
	//printf("ENTRAMOS A EAP_PEER_SM_STEP\n");
	fflush(stdout);
	int res = 0;
	do {
		sm->changed = FALSE;
		SM_STEP_RUN(EAP);
		//printf("DESPUES DE SM_STEP_RUN \n");
		fflush(stdout);
		if (sm->changed)
			res = 1;
