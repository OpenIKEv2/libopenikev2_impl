...
send_ra(int sock, struct Interface *iface, struct in6_addr *dest)
{
	...
	if(check_device(sock, iface) < 0) {
		if (iface->IgnoreIfMissing)  /* a bit more quiet warning message.. */
			fprintf(stderr, "radvd: interface %s does not exist, ignoring the interface\n", iface->Name);
		else {
			fprintf(stderr, "radvd: interface %s does not exist, ignoring the interface\n", iface->Name);
		}
		iface->HasFailed = 1;
		...
		if (iface->HasFailed == 1) {
			fprintf(stderr, "radvd: interface %s seems to have come back up, something bad is happening\n", iface->Name);
			iface->HasFailed = 0;
	...
	if (check_allrouters_membership(sock, iface) < 0)
		fprintf(stderr, "radvd: problem checking all-routers membership on %s\n", iface->Name);

	fprintf(stderr, "radvd: sending RA on %s\n", iface->Name);
	...
	if (err < 0) {
		if (!iface->IgnoreIfMissing || !(errno == EINVAL || errno == ENODEV))
			fprintf(stderr, "radvd: sendmsg: %s\n", strerror(errno));
		else
			fprintf(stderr, "radvd: sendmsg: %s\n", strerror(errno));
	}			
